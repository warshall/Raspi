...
static void sendDataCmd (const struct lcdDataStruct *lcd, const uint8_t data)
{
  register uint8_t myData = data ;
  uint8_t          i, d4 ;
...
static void putCommand (const struct lcdDataStruct *lcd, const uint8_t command)
{
  digitalWrite (lcd->rsPin,   0) ;
  sendDataCmd  (lcd, command) ;
}

static void put4Command (const struct lcdDataStruct *lcd, const uint8_t command)
{
  register uint8_t myCommand = command ;
  register uint8_t i ;
...
void lcdSendCommand (const int fd, const uint8_t command)
{
  struct lcdDataStruct *lcd = lcds [fd] ;
...
void lcdPosition (const int fd, const int x, const int y)
{
...
void lcdPutchar (const int fd, const uint8_t data)
{
  struct lcdDataStruct *lcd = lcds [fd] ;
