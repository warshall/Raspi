...
int gbWiringPiAnalogRead (struct wiringPiNodeStruct *node, const int chan)
{
  return gertboardAnalogRead (node->pinBase - chan) ;
}

void gbWiringPiAnalogWrite (struct wiringPiNodeStruct *node, const int chan, const int value)
{
  gertboardAnalogWrite (node->pinBase - chan, value) ;
}
